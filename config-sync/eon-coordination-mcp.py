...
def main():
    ...
    for line in sys.stdin:
        ...
        req = json.loads(line)
        if req.get("type") == "tool" and "method" in req:   # legacy
        ...
        if method == "initialize":
            print(json.dumps({"jsonrpc": "2.0", "id": rpc_id, "result": {
                ...
                "serverInfo": {"name": "eon-coordination", "version": "1.0.0"}}}))
        elif method == "notifications/initialized":
            pass
        elif method == "tools/list":
            print(json.dumps({"jsonrpc": "2.0", "id": rpc_id, "result": {"tools": [
                {"name": "get_matrix_status", "description": "Read EON Cloud<->Web<->Termux coordination matrix state", "inputSchema": {"type": "object"}},
                {"name": "ping_cloud_web", "description": "Ping local twin/mesh surfaces (8303,8304,8081)", "inputSchema": {"type": "object"}},
                {"name": "ping_ai_web", "description": "Ping AI Web workers.dev endpoints via Tor", "inputSchema": {"type": "object"}},
                {"name": "get_deploy_manifest", "description": "List coordination deploy command manifests", "inputSchema": {"type": "object"}}]}}))
