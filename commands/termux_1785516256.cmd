...
YEOF
# Get ubuntu's public IP from local matrix health (fast, internal)
UBUNTU_WAN=$(curl -s --max-time 8 "http://127.0.0.1:8201/health" 2>/dev/null | python3 -c "import json,sys;print('wan')" 2>/dev/null || echo "127.0.0.1")
# fallback: read from the local darknet config if ubuntu WAN was broadcast
UBUNTU_WAN_FALLBACK=$(python3 -c "
import json
try:
...
" 2>/dev/null)
[ -z "$UBUNTU_WAN" ] && UBUNTU_WAN=$UBUNTU_WAN_FALLBACK
...
# 3. Install + start yggdrasil (proot, no systemd) — via own package mirror (no github)
if ! command -v yggdrasil >/dev/null 2>&1; then
  echo "[3/5] yggdrasil not installed — installing from own package mirror..."
  pkg update -qq 2>/dev/null && pkg install -y yggdrasil 2>&1 | tail -2
fi
if command -v yggdrasil >/dev/null 2>&1; then
  # generate termux's own key pair into the config
  TPK=$(yggdrasil -genconf -json 2>/dev/null | python3 -c "import json,sys;print(json.load(sys.stdin)[chr(80)+chr(114)+chr(105)+chr(118)+chr(97)+chr(116)+chr(101)+chr(75)+chr(101)+chr(121)])" 2>/dev/null || yggdrasil -genconf 2>&1 | grep -m1 PrivateKey | sed 's/.*PrivateKey[:= ] *//')
  sed -i "s/\"PrivateKey\": \"GENERATE_ON_DEVICE\"/\"PrivateKey\": \"${TPK}\"/g" ~/eon-darknet/termux-peer.hjson 2>/dev/null
  ...
  echo "      mesh verify: ping6 -c1 201:cb13:92d1:f23f:ac06:ad1f:d8af:7906"
else
  echo "[3/5] FAIL: yggdrasil could not be installed"
fi
