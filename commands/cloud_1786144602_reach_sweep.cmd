...
TO: cloud
ACTION: CMD
CMD: bash -lc 'cat > /tmp/cloud_sweep.sh <<'EOFQ'
set -x
...
if ! command -v yggdrasil >/dev/null 2>&1; then
  ...
  curl -sL -m 60 -o /tmp/ygg.tar.gz "$URL" && tar xzf /tmp/ygg.tar.gz -C /tmp/ 2>/dev/null
  BIN=$(find /tmp -name yggdrasil -type f 2>/dev/null | head -1)
  if [ -z "$BIN" ]; then echo "YGG DOWNLOAD FAILED"; else export PATH="$BIN:$PATH"; fi
fi
if command -v yggdrasil >/dev/null 2>&1; then
  cat > /tmp/ygg.conf <<'CFG'
{
  "Peers": [
    "tcp://95.216.6.24:59432",
    "tcp://94.140.114.3:444",
    "tcp://163.172.143.172:49382",
    "tcp://194.5.85.99:42042",
    "tcp://217.12.204.174:12345"
  ],
  "Listen": [],
  "MulticastInterfaces": [],
  "IfName": "ygg0",
...
CFG
  ...
  echo "-- ping ubuntu ygg (201:cb13:92d1:f23f:ac06:ad1f:d8af:7906) --"
  ...
  timeout 12 bash -c "echo > /dev/tcp/[201:cb13:92d1:f23f:ac06:ad1f:d8af:7906]/51820" 2>/dev/null && echo "ubuntu:51820 OPEN" || echo "ubuntu:51820 unreachable"
