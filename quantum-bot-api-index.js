...
// ═══════════════════════════════════════════════════════════════════════
// CHAT CHANNEL UPGRADES
// ═══════════════════════════════════════════════════════════════════════
async function sendTyping(chatId) {
  try { await tgApi('sendChatAction', { chat_id: chatId, action: 'typing' }); } catch {}
}

async function sendMessageChunked(chatId, text, maxLen = 4000) {
  if (text.length <= maxLen) {
    await tgApi('sendMessage', { chat_id: chatId, text, parse_mode: 'Markdown' }).catch(() =>
      tgApi('sendMessage', { chat_id: chatId, text })  // fallback without markdown
    );
    return;
  }
  // Split at paragraph or line break
  const chunks = [];
  let remaining = text;
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    await tgApi('sendMessage', { chat_id: chatId, text: prefix + chunks[i] }).catch(() => {});
    if (i < chunks.length - 1) await new Promise(r => setTimeout(r, 500)); // rate limit
  }
}
...
function mainKeyboard() {
  return JSON.stringify({
    keyboard: [
      [{ text: '/quantum Ask anything' }, { text: '/status System' }],
      [{ text: '/debate Topic' }, { text: '/verify Fact-check' }],
      [{ text: '/code Task' }, { text: '/help Commands' }]
    ],
...
async function handleMessage(text, chatId, firstName) {
  ...
  if (cmd === '/start' || cmd === '/help') {
    await sendMessageChunked(chatId,
      `🧠 *EON AGI v8.0 — 8-Layer Intelligence*\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
      `*Commands:*\n` +
      `• /quantum <q> — Multi-brain consensus\n` +
      `• /debate <topic> — Thesis vs Antithesis\n` +
      `• /code <task> — Code generation\n` +
      `• /verify <text> — Fact-check\n` +
      `• /status — System stats\n` +
      `• /dream — Architecture vision\n\n` +
      `*Or just type anything — AGI activates automatically.*\n\n` +
      `Layers: TF-IDF memory, cross-model verify,\n` +
      `CoVe checking, AAD pre-drafting, causal chains,\n` +
      `confidence scoring. Mandatory execute — no objections.`
    );
    // Send keyboard
    await tgApi('sendMessage', { chat_id: chatId, text: 'Quick commands:', reply_markup: mainKeyboard() }).catch(() => {});
    return;
  }
  ...
  try {
    ...
    await sendMessageChunked(chatId, result.text);
    ...
    if (fallback) {
      await sendMessageChunked(chatId, `🛡 [Self-healed]\n\n${fallback.slice(0, 4000)}`);
    } else {
      await tgApi('sendMessage', { chat_id: chatId, text: `✗ Error: ${e.message.slice(0, 200)}\n\nTry /help for commands.` });
    }
  }
}
