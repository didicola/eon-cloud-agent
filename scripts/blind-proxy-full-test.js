...
// ─── Prometheus-style metrics (Step 4 of delegated build) ───
const METRICS = {
  requests_total: 0,
  success_total: 0,
  error_total: 0,
  by_model: {},          // model -> {requests, success, errors, latency_sum}
  by_provider: {},       // provider -> {requests, success, errors}
  start_time: Date.now(),
  last_latency_ms: 0,
};
function metricIncr(counter, key, delta) { return; }

// ─── Phase 4 Step N: multi-model auto-scaling ───
// Health score per model: high success + low latency + enough samples = better.
// Used to pick the best-performing $0 model for 'auto' requests and exposed via
// GET /v1/autoscale so the dashboard can show live ranking.
const SCALE = { by_model: {}, last_snapshot: null };
function modelHealthScore(m) {
  const s = METRICS.by_model[m];
  if (!s || s.requests < 3) return null; // not enough samples
  const successRate = s.success / s.requests;
  const avgLatency = s.latency_sum / s.requests;
  const latencyScore = Math.max(0, 1 - (avgLatency - 1000) / 29000); // 1s→1.0, 30s→0
  const volumeScore = Math.min(1, s.requests / 50);
  return {
    model: m,
    requests: s.requests,
    success_rate: +successRate.toFixed(3),
    avg_latency_ms: Math.round(avgLatency),
    score: +((successRate * 0.6 + latencyScore * 0.3 + volumeScore * 0.1)).toFixed(3),
  };
}
function autoscaleSnapshot() {
  const ranked = Object.keys(METRICS.by_model)
    .map(modelHealthScore)
    .filter(Boolean)
    .sort((a, b) => b.score - a.score);
  SCALE.last_snapshot = { ts: Date.now(), model_count: ranked.length, ranked };
  return SCALE.last_snapshot;
}
function bestModelFor(taskHint) {
  const snap = autoscaleSnapshot();
  if (!snap || snap.model_count === 0) return null;
  // prefer healthy models matching the task hint, else global best
  const hint = String(taskHint || '').toLowerCase();
  const match = snap.ranked.filter(h => h.model.toLowerCase().includes(hint) && h.success_rate > 0.8);
  return (match[0] || snap.ranked[0]).model;
}
...
http.createServer((req, res) => {
    ...
    (async () => {
      const [pingResults, inferenceResults] = await Promise.all([
        Promise.all([
          ...
          check('http://127.0.0.1:8091/v1/models'),
  ...
  if (req.method === 'GET' && req.url === '/v1/circuit') {
    const now = Date.now();
    const open = [], closed = [];
    for (const [k, v] of modelFailCache) {
      (now < v.expiresAt ? open : closed).push({ key: k, expiresAt: v.expiresAt, ttlMs: v.expiresAt - now });
    }
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({
      open: open.length, closed: closed.length,
      open_breakers: open.slice(0, 50), recovered: closed.slice(0, 50),
      window_ms: 120000, policy: '3-fail-in-window → block 120s/300s(429) → auto-recover'
    }));
  }

  // Prometheus-style metrics endpoint (Step 4)
  if (req.method === 'GET' && req.url === '/v1/metrics') {    const up = Math.round(process.uptime());
    const lines = [
      '# HELP eon_requests_total Total chat requests',
      '# TYPE eon_requests_total counter',
      'eon_requests_total ' + METRICS.requests_total,
      'eon_success_total ' + METRICS.success_total,
      'eon_error_total ' + METRICS.error_total,
      'eon_uptime_seconds ' + up,
      'eon_last_latency_ms ' + METRICS.last_latency_ms,
      'eon_models_served ' + loadModelIds().length,
      '',
      '# HELP eon_model_requests Per-model request counters',
      '# TYPE eon_model_requests gauge',
    ];
    for (const [m, s] of Object.entries(METRICS.by_model)) {
      const avg = s.requests ? Math.round(s.latency_sum / s.requests) : 0;
      lines.push('eon_model_requests{model="' + m.replace(/"/g, '\\"') + '"} ' + s.requests);
      lines.push('eon_model_success{model="' + m.replace(/"/g, '\\"') + '"} ' + s.success);
      lines.push('eon_model_errors{model="' + m.replace(/"/g, '\\"') + '"} ' + s.errors);
      lines.push('eon_model_avg_latency_ms{model="' + m.replace(/"/g, '\\"') + '"} ' + avg);
    }
    res.writeHead(200, { 'Content-Type': 'text/plain; version=0.0.4; charset=utf-8' });
    return res.end(lines.join('\n') + '\n');
  }

  // Phase 4 Step N: auto-scaling endpoint — live per-model health ranking
  if (req.method === 'GET' && req.url === '/v1/autoscale') {
    const snap = autoscaleSnapshot();
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, ts: snap.ts, model_count: snap.model_count, ranked: snap.ranked, recommended: bestModelFor(null) }));
  }
  ...
  if (req.method === 'POST' && req.url === '/v1/chat/completions') {
    ...
    const emitSpan = (status, extra) => {
      try {
        const p = JSON.parse(body || '{}');
        const tp = req.headers['traceparent'] || req.headers['x-trace-id'] || undefined;
        const s = { trace_id: traceId, traceparent: tp, name: 'blind-proxy/chat', service: 'blind-proxy', model: p.model || 'auto', status, duration_ms: Date.now() - traceStart, provider: extra?.provider, ...extra };
        fetch('http://127.0.0.1:8121/spans', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(s) }).catch(() => {});
      } catch {}
    };
    const origEnd = res.end.bind(res);
    let spanEmitted = false;
    const finishMetrics = (status) => {
      try {
        const p = JSON.parse(body || '{}');
        const m = p.model || 'auto';
        METRICS.requests_total++;
        const d = Date.now() - traceStart;
        METRICS.last_latency_ms = d;
        METRICS.by_model[m] = METRICS.by_model[m] || { requests: 0, success: 0, errors: 0, latency_sum: 0 };
        METRICS.by_model[m].requests++;
        METRICS.by_model[m].latency_sum += d;
        if (status === 'ok') { METRICS.success_total++; METRICS.by_model[m].success++; }
        else { METRICS.error_total++; METRICS.by_model[m].errors++; }
      } catch {}
    };
    ...
    res.on('close', () => {
      if (!spanEmitted) { spanEmitted = true; emitSpan('error', { err: 'connection closed before response' }); finishMetrics('error'); }
    });
    req.on('data', c => body += c);
    req.on('end', async () => {
      try {
...
// ─── AUTO mode: try multiple providers in order of preference (keyless first) ───
        if (reqModel === 'auto' || reqModel === 'auto:free') {
          ...
          // Phase 4 Step N: prefer the healthiest proven $0 model for this task
          const healthy = bestModelFor(inferTaskType && inferTaskType(parsed.messages));
          if (healthy) {
            selection.model = healthy;
            console.error('[autoscale] health pick: ' + healthy);
          }
...
Return the results as a concise dependency graph.`;
        ...
        const result = await fetchJSON('http://127.0.0.1:8091/v1/chat/completions',
  ...
  if (req.method === 'POST' && req.url === '/v1/consensus') {
    ...
    req.on('end', async () => {
      try {
        ...
        try {
          const judgeData = await fetchJSON('http://127.0.0.1:8091/v1/chat/completions', judgePayload, 60000);
...
// Phase 4 Step N: push auto-scaling health snapshot to own-cloud D1 every 60s
const AUTOSCALE_SPACE = 'ai-cloud-space.exportdefaultasyncfetchrequestenvconsturl.workers.dev';
const AUTOSCALE_TOKEN = '48e6a9a31a84f5b28d832a2e14dcf470a2ae15b20fbc0bd606e583991385b349';
setInterval(() => {
  try {
    const snap = autoscaleSnapshot();
    if (!snap || snap.model_count === 0) return;
    const body = JSON.stringify({ ts: snap.ts, model_count: snap.model_count, ranked: snap.ranked, recommended: bestModelFor(null) });
    const { execFile } = require('child_process');
    execFile('curl', ['-s', '--socks5-hostname', '127.0.0.1:9050', '--max-time', '6',
      '-X', 'PUT', '-H', 'Authorization: Bearer ' + AUTOSCALE_TOKEN, '-H', 'Content-Type: application/json',
      '--data-binary', body, 'https://' + AUTOSCALE_SPACE + '/d1/autoscale/latest'], (err) => {
      if (err) console.error('[autoscale] D1 push failed:', err.message);
    });
  } catch (e) {}
}, 60000);
console.log('[autoscale] health-based auto-scaling active (D1 sync 60s)');
